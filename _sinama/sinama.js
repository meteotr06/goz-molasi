...
(async () => {
  ...
  /* ---------- 3b. UZUN MOLA ---------- */
  {
    ...
    if (kart) {
      ...
      /* SABIT BEKLEME YOK — ISARET BEKLENIYOR.

         Once `setTimeout(..., 150)` vardi ve kartin o 150 ms icinde
         ACILDIGI varsayiliyordu. Kart mola ortusu kapandiktan sonra
         aciliyor; makine mesgulken 150 ms yetmiyor. O zaman dugmeler
         daha `hidden` bir kartin icinde olculuyor, yani yuksekligi
         SIFIR cikiyor ve sinama "2 dugme kucuk" diyordu.

         Olculdu (03.09.2026): setTimeout(0) aninda kart hidden=true ve
         yukseklikler [0, 0]; 600 ms sonra hidden=false ve [44, 44].
         Yani URUN dogruydu, KIRMIZI YANLISTI. Yanlis sebeple oten bir
         bekci, susan bekciden de kotudur: kirmiziyi onemsememeyi
         ogretir. Ayni sinif `sinama_aile.py` icinde de yasandi.

         Artik zamanlama varsayimi yok: kart acilana kadar (en cok
         3 sn) bekleniyor. Ustelik `await` sayesinde bu dort sonuc
         rapordan DUSEMIYOR — setTimeout ile rapor once bitseydi
         sessizce kaybolabilirlerdi. */
      const acilmayiBekle = async () => {
        for (let i = 0; i < 60; i++) {
          if (!kart.hidden) return true;
          await new Promise((r) => setTimeout(r, 50));
        }
        return false;
      };
      const acildi = await acilmayiBekle();
      ekle('uzun mola', 'olayın dinleyicisi var (kart açılıyor)', acildi,
           acildi ? '' : 'olay yayılıyor ama kimse dinlemiyor');
      ...
      if (!acildi) {
        /* Kart acilmadiysa olcum ANLAMSIZ. "Kucuk" demek yanlis sebep
           olurdu; ustteki satir zaten gercek kusuru soyluyor. */
        ekle('uzun mola', 'düğmeler 44 px yüksekliğinde', false,
             'ölçülemedi — kart açılmadı');
      } else {
