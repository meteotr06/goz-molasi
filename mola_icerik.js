...
const MolaIcerik = (() => {
  ...
  /* HAVA ADLARININ INGILIZCE IKIZI. Tablo yalniz Turkceydi ve dile
     hic bakilmadan kullaniliyordu: kartin GOVDESI Ingilizceyken
     BASLIGINDA "Parcali bulutlu" yaziyordu. */
  const HAVA_ADI_EN = {
  ...
  function havaKarti(h) {
    if (!h) return null;
    const [adTr, simge] = HAVA_ADI[h.kod] || ['—', '🌡️'];
    const ad = _ing() ? (HAVA_ADI_EN[h.kod] || adTr) : adTr;
    const yer = h.ad ? `${h.ad} · ` : '';
